...
namespace JModder
{
    public class JMod : MelonMod
    {
        ...
        public static bool SwitchMenu(bool isEscape)
        {
            if (!showCheatsPopup && !isEscape)
            {
        ...
        public static void ShowMenu()
        {
            if (showCheatsPopup)
            {
...
#if ENABLE_INPUT_SYSTEM
                if (jModWindowRect.Contains(mousePosition))
#else
                ...
                if (jModEventSys.IsPointerOverGameObject())
                {
        ...
        private static void ModMenuWindow(int windowId)
        {
            ...
            yAxis += 35;
            if (ToggleCategory.Entries.Count > 0)
            ...
            yAxis += 25;
            if (MultiplierFloatCategory.Entries.Count > 0)
                ShowFloatMenu(ref xAxis, ref yAxis, ref MultiplierFloatCategory);
            if (MultiplierIntCategory.Entries.Count > 0)
        ...
        {
            foreach (MelonPreferences_Entry<bool> toggle in cat.Entries.Cast<MelonPreferences_Entry<bool>>())
            {
        ...
        {
            foreach (MelonPreferences_Entry<float> mult in cat.Entries.Cast<MelonPreferences_Entry<float>>())
            {
        ...
        {
            foreach (MelonPreferences_Entry<int> mult in cat.Entries.Cast<MelonPreferences_Entry<int>>())
            {
                string multLabel = mult.DisplayName;
                ValueRange<int> range = (ValueRange<int>)mult.Validator;
                multLabel += " (" + range.MinValue.ToString() + " - " + range.MaxValue.ToString() + ")";
                GUI.Label(new Rect(xAxis, yAxis, 620, 20), multLabel, JModStyleP);
                if (GUI.Button(new Rect(xAxis + 680, yAxis, 40, 20), " - "))
                {
                    if (mult.Value > range.MinValue)
                        mult.Value -= 1;
                }
                GUI.Label(new Rect(xAxis + 730, yAxis, 20, 20), mult.Value.ToString(), JModStylePV);
                if (GUI.Button(new Rect(xAxis + 750, yAxis, 40, 20), " + "))
                {
